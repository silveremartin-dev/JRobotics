 ...
 * 
 * <p>Builds an occupancy grid map while simultaneously localizing the robot.
 * Uses beam sensor model for LIDAR integration.</p>
 * 
 * <p><b>References:</b></p>
 * <ul>
 *   <li>Thrun, S., Burgard, W., & Fox, D. (2005). <i>Probabilistic Robotics</i>. Chapter 9.</li>
 *   <li>Grisetti, G., Stachniss, C., & Burgard, W. (2007). Improved Techniques for 
...
public class OccupancyGridSLAM {
    ...
    public void integrateScan(double[] scanRanges, double[] scanAngles, double maxRange) {
        for (int i = 0; i < scanRanges.length; i++) {
            ...
            if (range <= 0 || range >= maxRange) continue;
    ...
    public double getOccupancy(int gx, int gy) {
        if (!isValidCell(gx, gy)) return 0.5;
    ...
    public byte[][] exportMap() {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                if (p > 0.7) map[x][y] = (byte) 255;
                else if (p < 0.3) map[x][y] = (byte) 128;
                else map[x][y] = 0;
            }
        }
        return map;
    ...
    private void normalizeTheta() {
        while (robotTheta > Math.PI) robotTheta -= 2 * Math.PI;
        while (robotTheta < -Math.PI) robotTheta += 2 * Math.PI;
    }
